...
std::optional<User> UserModel::findUserByAuthId(const std::string &auth_id)
{
    DatabaseConnection &db = DatabaseConnection::getInstance();
    MYSQL *conn = db.getConnection();
    if (!conn)
    ...
    const char *query = "SELECT id, first_name, email, password, auth_provider, auth_id, created_at FROM users WHERE auth_id = ?";
    MYSQL_STMT *stmt = mysql_stmt_init(conn);
    if (!stmt)
    {
        std::cerr << "Error: Falló la inicialización del statement: " << mysql_error(conn) << std::endl;
    ...
    // Vincular el parámetro de entrada (auth_id)
    MYSQL_BIND param[1];
    memset(param, 0, sizeof(param));

    param[0].buffer_type = MYSQL_TYPE_STRING;
    param[0].buffer = (void *)auth_id.c_str();
    param[0].buffer_length = auth_id.length();
    ...
    result[5].buffer_type = MYSQL_TYPE_STRING;
    result[5].buffer = auth_id_buf;
    result[5].buffer_length = sizeof(auth_id_buf);
    result[5].length = &auth_id_len;
    result[5].is_null = &is_null[5];
    ...
    user.password = std::string(password, password_len);
    user.auth_provider = std::string(auth_provider, auth_provider_len);
    user.auth_id = std::string(auth_id_buf, auth_id_len);
    user.created_at = std::string(created_at, created_at_len);
